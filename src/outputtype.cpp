...
static const std::string OUTPUT_TYPE_STRING_P2TSH = "p2tsh";
...
std::optional<OutputType> ParseOutputType(std::string_view type)
{
    ...
    } else if (type == OUTPUT_TYPE_STRING_P2TSH) {
...
const std::string& FormatOutputType(OutputType type)
{
    ...
    case OutputType::P2TSH: return OUTPUT_TYPE_STRING_P2TSH;
...
CTxDestination AddAndGetDestinationForScript(FlatSigningProvider& keystore, const CScript& script, OutputType type)
{
    ...
    } // This function should not be used for BECH32M, P2TSH or UNKNOWN, so let it assert
    } // no default case, so the compiler can warn about missing cases
    assert(false);
}
