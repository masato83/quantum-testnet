...
static void mld_polyvecl_permute_bitrev_to_custom(mld_polyvecl* v)
    __contract__(
         ...
         */
        requires(memory_no_alias(v, sizeof(mld_polyvecl))) requires(forall(x, 0, MLDSA_L,
...
static void mld_polymat_permute_bitrev_to_custom(mld_polymat* mat)
    __contract__(
         ...
         * mld_polyvec_matrix_expand.
         */
        requires(memory_no_alias(mat, sizeof(mld_polymat))) requires(forall(k1, 0, MLDSA_K, forall(l1, 0, MLDSA_L, array_bound(mat->vec[k1].vec[l1].coeffs, 0, MLDSA_N, 0, MLDSA_Q))))
                               ...
                               const uint8_t rho[MLDSA_SEEDBYTES])
{
    ...
    for (i = 0; i < (MLDSA_K * MLDSA_L / 4) * 4; i += 4)
        __loop__(
            ...
            invariant(forall(k1, 0, i / MLDSA_L, forall(l1, 0, MLDSA_L, array_bound(mat->vec[k1].vec[l1].coeffs, 0, MLDSA_N, 0, MLDSA_Q))))
            /* last vector is sampled up to i % MLDSA_L */
            invariant(forall(k2, i / MLDSA_L, i / MLDSA_L + 1, forall(l2, 0, i % MLDSA_L, array_bound(mat->vec[k2].vec[l2].coeffs, 0, MLDSA_N, 0, MLDSA_Q)))))
        {
            for (j = 0; j < 4; j++)
                __loop__(
                    assigns(j, object_whole(seed_ext))
