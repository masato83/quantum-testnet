...
static int mld_check_pct(uint8_t const pk[MLDSA_CRYPTO_PUBLICKEYBYTES],
    ...
    __contract__(
        requires(memory_no_alias(pk, MLDSA_CRYPTO_PUBLICKEYBYTES)) requires(memory_no_alias(sk, MLDSA_CRYPTO_SECRETKEYBYTES))
            ensures(return_value == 0 || return_value == MLD_ERR_FAIL || return_value == MLD_ERR_OUT_OF_MEMORY || return_value == MLD_ERR_RNG_FAIL));
...
static void mld_sample_s1_s2(mld_polyvecl* s1, mld_polyveck* s2,
    ...
    __contract__(
        requires(memory_no_alias(s1, sizeof(mld_polyvecl))) requires(memory_no_alias(s2, sizeof(mld_polyveck))) requires(memory_no_alias(seed, MLDSA_CRHBYTES))
...
static int mld_compute_t0_t1_tr_from_sk_components(
    ...
    MLD_CONFIG_CONTEXT_PARAMETER_TYPE context)
    __contract__(
        requires(memory_no_alias(t0, sizeof(mld_polyveck))) requires(memory_no_alias(t1, sizeof(mld_polyveck))) requires(memory_no_alias(tr, MLDSA_TRBYTES)) requires(memory_no_alias(pk, MLDSA_CRYPTO_PUBLICKEYBYTES)) requires(memory_no_alias(rho, MLDSA_SEEDBYTES)) requires(memory_no_alias(s1, sizeof(mld_polyvecl))) requires(memory_no_alias(s2, sizeof(mld_polyveck))) requires(forall(l0, 0, MLDSA_L, array_bound(s1->vec[l0].coeffs, 0, MLDSA_N, MLD_POLYETA_UNPACK_LOWER_BOUND, MLDSA_ETA + 1))) requires(forall(k0, 0, MLDSA_K, array_bound(s2->vec[k0].coeffs, 0, MLDSA_N, MLD_POLYETA_UNPACK_LOWER_BOUND, MLDSA_ETA + 1)))
                                ...
                                MLD_CONFIG_CONTEXT_PARAMETER_TYPE context)
{
    ...
    while (1)
        __loop__(
            ...
            invariant(forall(k1, 0, MLDSA_K, forall(l1, 0, MLDSA_L, array_bound(mat->vec[k1].vec[l1].coeffs, 0, MLDSA_N, 0, MLDSA_Q))))
