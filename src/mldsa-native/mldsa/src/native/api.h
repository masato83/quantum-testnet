...
static MLD_INLINE int mld_ntt_native(int32_t p[MLDSA_N])
    __contract__(
        requires(memory_no_alias(p, sizeof(int32_t) * MLDSA_N)) requires(array_abs_bound(p, 0, MLDSA_N, MLDSA_Q))
...
static MLD_INLINE void mld_poly_permute_bitrev_to_custom(int32_t p[MLDSA_N])
    __contract__(
         ...
         */
        requires(memory_no_alias(p, sizeof(int32_t) * MLDSA_N)) requires(array_bound(p, 0, MLDSA_N, 0, MLDSA_Q))
            assigns(memory_slice(p, sizeof(int32_t) * MLDSA_N))
...
static MLD_INLINE int mld_rej_uniform_native(int32_t* r, unsigned len,
                                             ...
                                             unsigned buflen)
    __contract__(
        requires(len <= MLDSA_N) requires(buflen <= (5 * 168) && buflen % 3 == 0) requires(memory_no_alias(r, sizeof(int32_t) * len)) requires(memory_no_alias(buf, buflen))
...
static MLD_INLINE int mld_rej_uniform_eta2_native(int32_t* r, unsigned len,
                                                  ...
                                                  unsigned buflen)
    __contract__(
        requires(len <= MLDSA_N) requires(buflen <= (2 * 136)) requires(memory_no_alias(r, sizeof(int32_t) * len)) requires(memory_no_alias(buf, buflen))
            assigns(memory_slice(r, sizeof(int32_t) * len))
                ensures(return_value == MLD_NATIVE_FUNC_FALLBACK || (0 <= return_value && return_value <= len))
...
static MLD_INLINE int mld_poly_decompose_32_native(int32_t* a1, int32_t* a0)
    __contract__(
        requires(memory_no_alias(a1, sizeof(int32_t) * MLDSA_N)) requires(memory_no_alias(a0, sizeof(int32_t) * MLDSA_N)) requires(array_bound(a0, 0, MLDSA_N, 0, MLDSA_Q))
...
static MLD_INLINE int mld_poly_caddq_native(int32_t a[MLDSA_N])
    __contract__(
        requires(memory_no_alias(a, sizeof(int32_t) * MLDSA_N)) requires(array_abs_bound(a, 0, MLDSA_N, MLDSA_Q))
...
static MLD_INLINE int mld_poly_use_hint_32_native(int32_t* b, const int32_t* a,
                                                  const int32_t* h)
    __contract__(
        requires(memory_no_alias(a, sizeof(int32_t) * MLDSA_N)) requires(memory_no_alias(b, sizeof(int32_t) * MLDSA_N)) requires(memory_no_alias(h, sizeof(int32_t) * MLDSA_N)) requires(array_bound(a, 0, MLDSA_N, 0, MLDSA_Q)) requires(array_bound(h, 0, MLDSA_N, 0, 2))
...
static MLD_INLINE int mld_poly_chknorm_native(const int32_t* a, int32_t B)
    __contract__(
        requires(memory_no_alias(a, sizeof(int32_t) * MLDSA_N)) requires(0 <= B && B <= MLDSA_Q - REDUCE32_RANGE_MAX) requires(array_bound(a, 0, MLDSA_N, -REDUCE32_RANGE_MAX, REDUCE32_RANGE_MAX))
            ensures(return_value == MLD_NATIVE_FUNC_FALLBACK || return_value == MLD_NATIVE_FUNC_SUCCESS)
...
static MLD_INLINE int mld_polyz_unpack_17_native(int32_t* r, const uint8_t* a)
    __contract__(
        requires(memory_no_alias(r, sizeof(int32_t) * MLDSA_N)) requires(memory_no_alias(a, MLDSA_POLYZ_PACKEDBYTES))
...
static MLD_INLINE int mld_poly_pointwise_montgomery_native(
    ...
    __contract__(
        requires(memory_no_alias(a, sizeof(int32_t) * MLDSA_N)) requires(memory_no_alias(b, sizeof(int32_t) * MLDSA_N)) requires(memory_no_alias(c, sizeof(int32_t) * MLDSA_N)) requires(array_abs_bound(a, 0, MLDSA_N, MLD_NTT_BOUND)) requires(array_abs_bound(b, 0, MLDSA_N, MLD_NTT_BOUND))
...
static MLD_INLINE int mld_polyvecl_pointwise_acc_montgomery_l4_native(
    ...
    __contract__(
        requires(memory_no_alias(w, sizeof(int32_t) * MLDSA_N)) requires(memory_no_alias(u, sizeof(int32_t) * 4 * MLDSA_N)) requires(memory_no_alias(v, sizeof(int32_t) * 4 * MLDSA_N)) requires(forall(l0, 0, 4,
                                                                                                                                                                                                        array_bound(u[l0], 0, MLDSA_N, 0, MLDSA_Q))) requires(forall(l1, 0, 4,
...
static MLD_INLINE int mld_polyvecl_pointwise_acc_montgomery_l5_native(
    ...
    __contract__(
        requires(memory_no_alias(w, sizeof(int32_t) * MLDSA_N)) requires(memory_no_alias(u, sizeof(int32_t) * 5 * MLDSA_N)) requires(memory_no_alias(v, sizeof(int32_t) * 5 * MLDSA_N)) requires(forall(l0, 0, 5,
                                                                                                                                                                                                        array_bound(u[l0], 0, MLDSA_N, 0, MLDSA_Q))) requires(forall(l1, 0, 5,
                                                                                                                                                                                                                                                                     array_abs_bound(v[l1], 0, MLDSA_N, MLD_NTT_BOUND)))
            assigns(memory_slice(w, sizeof(int32_t) * MLDSA_N))
                ensures(return_value == MLD_NATIVE_FUNC_FALLBACK || return_value == MLD_NATIVE_FUNC_SUCCESS)
                    ensures((return_value == MLD_NATIVE_FUNC_SUCCESS) == > array_abs_bound(w, 0, MLDSA_N, MLDSA_Q))
...
static MLD_INLINE int mld_polyvecl_pointwise_acc_montgomery_l7_native(
    ...
    __contract__(
        requires(memory_no_alias(w, sizeof(int32_t) * MLDSA_N)) requires(memory_no_alias(u, sizeof(int32_t) * 7 * MLDSA_N)) requires(memory_no_alias(v, sizeof(int32_t) * 7 * MLDSA_N)) requires(forall(l0, 0, 7,
                                                                                                                                                                                                        array_bound(u[l0], 0, MLDSA_N, 0, MLDSA_Q))) requires(forall(l1, 0, 7,
                                                                                                                                                                                                                                                                     array_abs_bound(v[l1], 0, MLDSA_N, MLD_NTT_BOUND)))
            assigns(memory_slice(w, sizeof(int32_t) * MLDSA_N))
                ensures(return_value == MLD_NATIVE_FUNC_FALLBACK || return_value == MLD_NATIVE_FUNC_SUCCESS)
                    ensures((return_value == MLD_NATIVE_FUNC_SUCCESS) == > array_abs_bound(w, 0, MLDSA_N, MLDSA_Q))
