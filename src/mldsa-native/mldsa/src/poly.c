...
static MLD_INLINE void mld_ntt_butterfly_block(int32_t r[MLDSA_N],
    ...
    __contract__(
        requires(start < MLDSA_N) requires(1 <= len && len <= MLDSA_N / 2 && start + 2 * len <= MLDSA_N) requires(0 <= bound && bound < INT32_MAX - MLDSA_Q) requires(-MLDSA_Q_HALF < zeta && zeta < MLDSA_Q_HALF) requires(memory_no_alias(r, sizeof(int32_t) * MLDSA_N)) requires(array_abs_bound(r, 0, start, bound + MLDSA_Q)) requires(array_abs_bound(r, start, MLDSA_N, bound))
...
static MLD_INLINE void mld_ntt_layer(int32_t r[MLDSA_N], const unsigned layer)
    __contract__(
        requires(memory_no_alias(r, sizeof(int32_t) * MLDSA_N)) requires(1 <= layer && layer <= 8) requires(array_abs_bound(r, 0, MLDSA_N, layer* MLDSA_Q))
            assigns(memory_slice(r, sizeof(int32_t) * MLDSA_N))
...
static MLD_INLINE void mld_invntt_layer(int32_t r[MLDSA_N], unsigned layer)
    __contract__(
        requires(memory_no_alias(r, sizeof(int32_t) * MLDSA_N)) requires(1 <= layer && layer <= 8) requires(array_abs_bound(r, 0, MLDSA_N, (MLDSA_N >> layer) * MLDSA_Q))
            assigns(memory_slice(r, sizeof(int32_t) * MLDSA_N))
...
MLD_STATIC_TESTABLE unsigned int mld_rej_uniform_c(int32_t* a,
                                                   ...
                                                   unsigned int buflen)
    __contract__(
        requires(offset <= target && target <= MLDSA_N) requires(buflen <= (MLD_POLY_UNIFORM_NBLOCKS * MLD_STREAM128_BLOCKBYTES) && buflen % 3 == 0) requires(memory_no_alias(a, sizeof(int32_t) * target)) requires(memory_no_alias(buf, buflen)) requires(array_bound(a, 0, offset, 0, MLDSA_Q))
