...
void mld_shake128x4_absorb_once(mld_shake128x4ctx* state, const uint8_t* in0,
    ...
    __contract__(
        requires(inlen <= MLD_MAX_BUFFER_SIZE) requires(memory_no_alias(state, sizeof(mld_shake128x4ctx))) requires(memory_no_alias(in0, inlen)) requires(memory_no_alias(in1, inlen)) requires(memory_no_alias(in2, inlen)) requires(memory_no_alias(in3, inlen))
...
void mld_shake128x4_squeezeblocks(uint8_t* out0, uint8_t* out1, uint8_t* out2,
    ...
    __contract__(
        requires(nblocks <= 8 /* somewhat arbitrary bound */) requires(memory_no_alias(state, sizeof(mld_shake128x4ctx))) requires(memory_no_alias(out0, nblocks* SHAKE128_RATE)) requires(memory_no_alias(out1, nblocks* SHAKE128_RATE)) requires(memory_no_alias(out2, nblocks* SHAKE128_RATE)) requires(memory_no_alias(out3, nblocks* SHAKE128_RATE))
...
void mld_shake256x4_absorb_once(mld_shake256x4ctx* state, const uint8_t* in0,
                                ...
                                const uint8_t* in3, size_t inlen)
    __contract__(
        requires(inlen <= MLD_MAX_BUFFER_SIZE) requires(memory_no_alias(state, sizeof(mld_shake256x4ctx))) requires(memory_no_alias(in0, inlen)) requires(memory_no_alias(in1, inlen)) requires(memory_no_alias(in2, inlen)) requires(memory_no_alias(in3, inlen))
...
void mld_shake256x4_squeezeblocks(uint8_t* out0, uint8_t* out1, uint8_t* out2,
    ...
    __contract__(
        requires(nblocks <= 8 /* somewhat arbitrary bound */) requires(memory_no_alias(state, sizeof(mld_shake256x4ctx))) requires(memory_no_alias(out0, nblocks* SHAKE256_RATE)) requires(memory_no_alias(out1, nblocks* SHAKE256_RATE)) requires(memory_no_alias(out2, nblocks* SHAKE256_RATE)) requires(memory_no_alias(out3, nblocks* SHAKE256_RATE))
