...
static unsigned int keccak_absorb(uint64_t s[MLD_KECCAK_LANES],
    ...
    __contract__(
        requires(inlen <= MLD_MAX_BUFFER_SIZE) requires(r < sizeof(uint64_t) * MLD_KECCAK_LANES) requires(pos <= r) requires(memory_no_alias(s, sizeof(uint64_t) * MLD_KECCAK_LANES)) requires(memory_no_alias(in, inlen))
            assigns(memory_slice(s, sizeof(uint64_t) * MLD_KECCAK_LANES))
...
static void keccak_finalize(uint64_t s[MLD_KECCAK_LANES], unsigned int pos,
    ...
    __contract__(
        requires(pos <= r && r < sizeof(uint64_t) * MLD_KECCAK_LANES) requires((r / 8) >= 1) requires(memory_no_alias(s, sizeof(uint64_t) * MLD_KECCAK_LANES))
            assigns(memory_slice(s, sizeof(uint64_t) * MLD_KECCAK_LANES)))
{
    uint8_t b = 0x80;
...
static unsigned int keccak_squeeze(uint8_t* out, size_t outlen,
    ...
    __contract__(
        requires((r == SHAKE128_RATE && pos <= SHAKE128_RATE) ||
                 ...
                 (r == SHA3_512_RATE && pos <= SHA3_512_RATE)) requires(outlen <= 8 * r /* somewhat arbitrary bound */) requires(memory_no_alias(s, sizeof(uint64_t) * MLD_KECCAK_LANES)) requires(memory_no_alias(out, outlen))
            assigns(memory_slice(s, sizeof(uint64_t) * MLD_KECCAK_LANES))
